...
var TimeGrid = FC.TimeGrid = Grid.extend(DayTableMixin, {
	...
	renderSlatRowHtml: function() {
		...
		while (slotTime < this.view.maxTime) {
		    ...
		    if (isLabeled) {
                // SETTINGOPTION : MAXPLACES
		        if ((slotDate.format('HH:mm:ss') > '00:00:00') && (slotDate.format('HH:mm:ss') < '18:00:00')) {
		            axisHtml =
					...
					if (buttonName == 'title') {
						...
						if (buttonClick) {
							...
							button = $( // type="button" so that it doesn't submit a form
								'<button type="button" class="btn btn-link ' + classes.join(' ') + '">' +
									innerHtml +
								'</button>'
								)
								.click(function(ev) {
